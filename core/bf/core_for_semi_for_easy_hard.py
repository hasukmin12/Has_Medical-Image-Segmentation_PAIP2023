...
def train_for_semi_easy_hard(info, config, global_step, dice_val_best, 
    ...
    zip_loader = zip(train_loader, easy_un_loader, hard_un_loader)
    ...
    loader_num = len(train_loader) + len(easy_un_loader)
    weight_scheduler = ConsistencyWeight(loader_num)

    epoch_iterator = tqdm(
        zip_loader, desc="Training (X / X Steps) (loss=X.X)", dynamic_ncols=True
    )
    for steps, (batch, e_un_batch, h_un_batch) in enumerate(epoch_iterator):
        steps += 1
        ...
        eu_x = e_un_batch["image"].to("cuda")
        hu_x = h_un_batch["image"].to("cuda")
        logit_map = model(hu_x)
        with torch.no_grad():
            t_logit_map = ema_model(eu_x)
        ...
        update_ema_variables(model, ema_model, 0.99, global_step=steps) # val 영향
