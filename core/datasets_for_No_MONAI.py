...
def call_dataloader_Lunit_single(info, config, data_list, label_list, transforms, mode='train'):
...
def call_dataloader_Lunit(info, config, data_list, label_list, t_data_list, c_data_list, transforms, mode='train'):
    torch.manual_seed(config["SEEDS"])
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False
    np.random.seed(config["SEEDS"])
