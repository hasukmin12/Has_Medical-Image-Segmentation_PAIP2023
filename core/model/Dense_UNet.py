...
from core.model.Conv_blocks_for_Dense import CoordAtt, DenseDownLayer, DenseDownBlock, DenseUpBlock, DenseUpLayer , DenseDownLayer_first, DenseDownBlock_first

__all__ = ["Dense_Unet", "dunet", "dunet", "D_Unet"]
...
class Dense_Encoder(nn.Module):
    def __init__(self, in_channel, channel_list, kernel_size, drop_rate):
        ...
        self.initial_conv = nn.Conv2d(in_channel, initial_channel, 3, padding=1)
        self.initial_norm = nn.BatchNorm2d(initial_channel, eps=1e-05, momentum=0.1, affine=True, track_running_stats=False)
        self.initial_nonlin = nn.LeakyReLU(negative_slope=0.01, inplace=True) 

        current_stage = DenseDownLayer_first(initial_channel, initial_channel, kernel_size, drop_rate)
        ...
        for stage in range(1, len(channel_list)):
            ...
            current_stage = DenseDownLayer(current_input_feature, current_output_feature,  kernel_size, drop_rate)
            self.stages.append(current_stage)
...
class Dense_Decoder(nn.Module):
    def __init__(self, out_channel, channel_list, kernel_size, drop_rate):
        ...
        for stage in range(len(channel_list)-1, 0, -1):
            ...
            self.trans_conv.append(transpconv(current_input_feature, current_output_feature, kernel_size=(2,2), stride=(2,2), bias=False))
            current_stage = DenseUpLayer(current_input_feature, current_output_feature, kernel_size, drop_rate)
            self.stages.append(current_stage)

        self.segmentation_output = nn.Conv2d(channel_list[0], out_channel, kernel_size=[1 for _ in kernel_size], stride=[1 for i in kernel_size], padding=[0 for i in kernel_size], bias=False)
...
class Dense_UNet(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, channel_list, kernel_size, drop_rate):
        ...
        self.encoder = Dense_Encoder(in_channel, channel_list, kernel_size, drop_rate)
        self.decoder = Dense_Decoder(out_channel, channel_list, kernel_size, drop_rate)
