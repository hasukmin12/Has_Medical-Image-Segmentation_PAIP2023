...
from core.model.ConvNextV2_utils import LayerNorm, GRN
...
class ConvNext_block(nn.Module):
    def __init__(self, ch_in, ch_out):
        super().__init__()
        self.conv_next = nn.Sequential(Block(dim=ch_out))
...
class ConvNeXtV2_UNet(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        self.Up4 = UpConvNext2(ch_in=dims[3], ch_out=dims[2])
        self.Up_conv4 = ConvNext_block(ch_in=dims[3], ch_out=dims[2])

        self.Up3 = UpConvNext2(ch_in=dims[2], ch_out=dims[1])
        self.Up_conv3 = ConvNext_block(ch_in=dims[2], ch_out=dims[1])

        self.Up2 = UpConvNext2(ch_in=dims[1], ch_out=dims[0])
        self.Up_conv2 = ConvNext_block(ch_in=dims[1], ch_out=dims[0])

        self.Up1 = UpConvNext2(ch_in=channels * 2, ch_out=channels)
        self.Up_conv1 = ConvNext_block(ch_in=channels * 2, ch_out=channels)
    ...
    def forward(self, x):
        ...
        x2 = self.downsample_layers[0](x1)
        x2 = self.stages[1](x2)
        
        x3 = self.downsample_layers[0](x2)
        x3 = self.stages[2](x3)

        x4 = self.downsample_layers[0](x3)
        x4 = self.stages[3](x4)
