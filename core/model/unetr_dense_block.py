...
class UnetDenseBlock(nn.Module):
    ...
    def forward(self, inp):
        ...
        concat1 = torch.cat((inp, out), dim=1)
        out = self.conv2(concat1)
        out = self.norm2(out)

        concat2 = torch.cat((out, concat1), dim=1)
        out = self.conv1_1(concat2)
...
class Dense_UnetrUpBlock(nn.Module):
    """
    ...
    def __init__(
        ...
        if dense_block:
            self.conv_block = UnetDenseBlock(
                spatial_dims,
...
class Dense_UnetrBlock(nn.Module):
    """
