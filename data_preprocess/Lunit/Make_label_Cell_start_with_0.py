...
# anno_path = '/vast/AI_team/sukmin/datasets/Lunit_Norm/annotations/train/cell'
# anno_list = sorted(next(os.walk(anno_path))[2])

# aim_path = '/vast/AI_team/sukmin/datasets/Lunit_Norm/annotations/train/cell_seg'
# os.makedirs(aim_path, exist_ok=True)

# output_path_for_visualize = '/vast/AI_team/sukmin/datasets/Lunit_Norm/annotations/train/cell_seg_visulization'
...
#             if cls == 1:
#                 cv2.circle(seg, (x,y), 7, 1, -1)
#             elif cls == 2:
#                 cv2.circle(seg, (x,y), 7, 2, -1)

#         print(seg.max())

#         cv2.imwrite(seg.astype(np.uint8), join(aim_path, 'cell_{0:03d}.png'.format(anno_list.index(case))))




import shutil
img_path = '/vast/AI_team/sukmin/datasets/Lunit_Challenge/Lunit_Norm/images/train/cell'
img_list = sorted(next(os.walk(img_path))[2])

aim_img_path = '/vast/AI_team/sukmin/datasets/Lunit_Challenge/Lunit_Norm/images/train/cell_start_0_case'
...
for case in img_list:
    ...
    # cv2.imwrite(rst, join(output_path_for_visualize, 'cell_{0:03d}.png'.format(anno_list.index(case))))
