...
aim_path = '/vast/AI_team/sukmin/datasets/Lunit_Challenge/Lunit_Not_norm/annotations/train/cell_seg_11'
os.makedirs(aim_path, exist_ok=True)

output_path_for_visualize = '/vast/AI_team/sukmin/datasets/Lunit_Challenge/Lunit_Not_norm/annotations/train/cell_seg_visulization_11'
...
for case in anno_list:
    if case != 'Thumbs.db':
        ...
        for line in pt:
            ...
            if cls == 1:
                cv2.circle(seg, (x,y), 11, 1, -1)
            elif cls == 2:
                cv2.circle(seg, (x,y), 11, 2, -1)

        # print(seg.max())

        cv2.imwrite(seg.astype(np.uint8), join(aim_path, 'case_{0:03d}.png'.format(anno_list.index(case))))
