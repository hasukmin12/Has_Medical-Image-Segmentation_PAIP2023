...
aim_path = '/vast/AI_team/sukmin/datasets/Lunit_Challenge/Task723_Lunit_Cell_StainNorm_rad10/labelsTs_circle_vis'
os.makedirs(aim_path, exist_ok=True)

if "Thumbs.db" in img_list:
    img_list.remove("Thumbs.db")

for case in anno_list[:80]:
    if case != 'Thumbs.db':
        print(case)
        ...
        img = cv2.imread(join(img_path, img_list[anno_list.index(case)]))
        ...
        Image.fromarray(img).save(join(aim_path, 'case_{0:03d}.png'.format(anno_list.index(case))))
