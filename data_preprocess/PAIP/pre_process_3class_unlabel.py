...
def main():
    ...
    parser.add_argument('-i', '--input_path', default='/home/sukmin/datasets/Task011_NeurIPS_Cellseg_unlabeled_tif', type=str, help='training data path; subfolders: images, labels')
    parser.add_argument("-o", '--output_path', default='/home/sukmin/datasets/Task011_NeurIPS_Cellseg_unlabeled', type=str, help='preprocessing data path')    
    args = parser.parse_args()
    
    source_path = args.input_path
    target_path = args.output_path
    
    img_path = join(source_path, 'imagesTr')
    ...
    for img_name in zip(tqdm(img_names)):
        if img_name[0].endswith('.tif') or img_name[0].endswith('.tiff'):
            img_data = tif.imread(join(img_path, img_name[0]))
        else:
            img_data = io.imread(join(img_path, img_name[0]))
