...
def main():
    ...
    parser.add_argument('-i', '--input_path', default='/vast/AI_team/sukmin/datasets/Train_Unlabel_data_pathes', type=str, help='training data path; subfolders: images, labels')
    parser.add_argument("-o", '--output_path', default='/home/sukmin/datasets/Task061_NeurIPS_Cellseg_unlabeled_patches', type=str, help='preprocessing data path')    
    ...
    pre_img_path = join(target_path, 'imagesTr')
    # pre_gt_path = join(target_path, 'labels')
    os.makedirs(pre_img_path, exist_ok=True)
    # os.makedirs(pre_gt_path, exist_ok=True)

    ref_path = '/home/sukmin/datasets/Task021_NeurIPS_Cellseg_unlabeled/imagesTr'
    
    for img_name in tqdm(img_names[125:]):
        if img_name.endswith('.tif') or img_name.endswith('.tiff'):
            img_file = join(ref_path, img_name.split('.')[0]+'.png')
            shutil.copy(img_file, join(target_path, 'imagesTr', img_name.split('.')[0]+'.png'))
        ...
        io.imsave(join(target_path, 'imagesTr', img_name[0].split('.')[0]+'.png'), pre_img_data.astype(np.uint8), check_contrast=False)
