...
def main():
    ...
    parser.add_argument("-o", '--output_path', default='/vast/AI_team/sukmin/datasets/Task_Stroma_prep_100x_whole_2', type=str, help='preprocessing data path')    
            ...
            for case in case_list:
                if case != 'Thumbs.db':
                    ...
                    if case[-9:] != 'label.png':
                        ...
                        patch_size = 512
                        patch_term = 435 # 85%                  # 384 - 75%          # 256 - 50% 
                        ...
                        if 0<=case_num<=27 or  143<=case_num<=148 or 170<=case_num<=173 or 187<=case_num<=191:
                            cv2.imwrite(image.astype(np.uint8), join(aim_path, 'val_images', 'case_{0:04d}.png'.format(case_num)))
                            cv2.imwrite(seg.astype(np.uint8), join(aim_path, 'val_labels', 'case_{0:04d}.png'.format(case_num)))

                        else:
                            cv2.imwrite(image.astype(np.uint8), join(aim_path, 'images', 'case_{0:04d}.png'.format(case_num)))
                            cv2.imwrite(seg.astype(np.uint8), join(aim_path, 'labels', 'case_{0:04d}.png'.format(case_num)))
