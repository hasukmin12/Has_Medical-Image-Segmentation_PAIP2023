...
def main():
    ...
    parser.add_argument('-i', '--input_path', default='/vast/AI_team/sukmin/datasets/2차annotation/100x(Downsample2)', type=str, help='training data path; subfolders: images, labels')
    parser.add_argument("-o", '--output_path', default='/vast/AI_team/sukmin/datasets/Task_Stroma_prep_100x_no_overlap_io_contrast_not_norm', type=str, help='preprocessing data path')    
            ...
            for case in case_list:
                if case != 'Thumbs.db':
                    ...
                    if case[-9:] != 'label.png':
                        ...
                        patch_size = 512
                        patch_term = 512             # 435 # 85%            # 384 - 75%          # 256 - 50%            # 128  - 25%
                        ...
                        for i in range(len(img_patches)):
                            ...
                            if 0<=case_num<=27 or  143<=case_num<=148 or 170<=case_num<=173 or 187<=case_num<=191:
                                io.imsave(join(aim_path, 'val_images', 'case_{0:04d}_{1:03d}.png'.format(case_num, i)), img.astype(np.uint8), check_contrast=True)
                                io.imsave(join(aim_path, 'val_labels', 'case_{0:04d}_{1:03d}.png'.format(case_num, i)), seg.astype(np.uint8), check_contrast=True)
                                ...
                                io.imsave(join(aim_path, 'images', 'case_{0:04d}_{1:03d}.png'.format(case_num, i)), img.astype(np.uint8), check_contrast=True)
                                io.imsave(join(aim_path, 'labels', 'case_{0:04d}_{1:03d}.png'.format(case_num, i)), seg.astype(np.uint8), check_contrast=True)
