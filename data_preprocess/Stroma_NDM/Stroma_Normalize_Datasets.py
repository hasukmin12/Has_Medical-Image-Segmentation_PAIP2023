...
def main():
    ...
    parser.add_argument('-i', '--input_path', default='/vast/AI_team/sukmin/datasets/Task_Stroma', type=str, help='training data path; subfolders: images, labels')
    parser.add_argument("-o", '--output_path', default='/vast/AI_team/sukmin/datasets/Task_Stroma_prep3', type=str, help='preprocessing data path')    
    ...
    for img_name, gt_name in zip(tqdm(img_names), gt_names):
        ...
        io.imsave(join(target_path, 'images', img_name), test_npy01.astype(np.uint8), check_contrast=False)
        io.imsave(join(target_path, 'labels', gt_name), gt_data.astype(np.uint8), check_contrast=False)
