...
def main(info, config, args):
    ...
    img_list = [{"image": join(args.input_path, img)} for img in img_names]
    t_img_list = [{"image": join(args.tissue_input_path, img)} for img in t_img_names]
    c_img_list = [{"image": join(args.cell_input_path, img)} for img in c_img_names]
    ...
    post_transforms = Compose(
        [
            Activationsd(keys="pred", softmax=True), # sigmoid=True),
            Invertd(
    ...
    dataset = Dataset_Lunit(data=img_list, t_data=t_img_list , c_data=c_img_list , transform=pre_transforms, v_transform=pre_transforms)
    ...
    with torch.no_grad():
        ...
        for d in dataloader:
            img, t_img, c_img = d
            img_data = img["image"].to(device)
            tissue_data = t_img["image"].to(device)
            cell_data = c_img["image"].to(device)

            t0 = time.time()

            test_npy01 = torch.cat((img_data, tissue_data, cell_data), dim=1)
            ...
            test_pred_out = sliding_window_inference(test_tensor, config["INPUT_SHAPE"], sw_batch_size, model)
            ...
            img_name = img['image_meta_dict']['filename_or_obj'][0].split('/')[-1]
            cv2.imwrite(join(args.output_path, img_name), rst)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', dest='trainer', default='Lunit_Tissue_with_YOLO_StainNorm_rad10_Blob_Cell') # 'Stroma_NDM_100x_overlap')                                                             # 2022_stomach_400x
    parser.add_argument('-i', dest='input_path', default='/vast/AI_team/sukmin/datasets/Lunit_Challenge/Task740_Lunit_Cell_StainNorm_r10_Blob_Cell/imagesTs') # Task701_Lunit_Cell_only/For_inference/imagesTs  
    parser.add_argument('-o', dest='aim_path', default="Lunit_Tissue_with_YOLO_StainNorm_Blob_Cell_DL_se_ResNext_b8_r10_div255_b10k") # Test_200x_CacoX_b32_e45k  
    parser.add_argument('-n', dest='epoch_num', default='model_best_e10001.pth') # model_e15000.pth # model_best_6296.pth
    ...
    parser.add_argument('-i_t', dest='tissue_input_path', default='/vast/AI_team/sukmin/datasets/Lunit_Challenge/Task740_Lunit_Cell_StainNorm_r10_Blob_Cell/Lunit_Tissue_cutting_Upscaling_255/labelsTs')
    ...
    parser.add_argument('-i_c', dest='cell_input_path', default='/vast/AI_team/sukmin/datasets/Lunit_Challenge/Task740_Lunit_Cell_StainNorm_r10_Blob_Cell/labelsTs_circle_150')
