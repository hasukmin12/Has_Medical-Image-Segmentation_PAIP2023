...
def main(info, config, args):
    ...
    c_model = YOLO(args.cell_model_path) # .to(device)
    sw_batch_size = 1
    ...
    with torch.no_grad():
        for img_name in img_names:
            if img_name != 'Thumbs.db':
                ...
                # Cell prediction
                cell_pred_out = c_model.predict(source=c_tensor, conf=0.5)

                # Draw cell segmentation map
                coord = cell_pred_out[0].boxes.data
                cell_seg = np.zeros((1024, 1024), dtype=np.uint8)
                for box in coord:
                    # print(box)
                    ...
                    cls = int(box[5])
                    if cls == 1:
                        cv2.circle(cell_seg, (x,y), 10, 255, -1)
                    elif cls == 2:
                        cv2.circle(cell_seg, (x,y), 10, 255, -1)
                ...
                # save for check
                Image.fromarray(cell_seg*100).save(join(check_path, "cell_" + img_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', dest='trainer', default='Lunit_Tissue_with_YOLO_StainNorm_rad10_Blob_Cell_No_MONAI') # 'Stroma_NDM_100x_overlap')                                                             # 2022_stomach_400x
    ...
    parser.add_argument('-o', dest='aim_path', default="Lunit_Tissue_with_YOLO_StainNorm_Blob_Cell_DL_se_ResNext_b8_r10_div255_No_MONAI_Real_data_Aug_Test") # Test_200x_CacoX_b32_e45k  
    parser.add_argument('-n', dest='epoch_num', default='model_best_e00100.pth') # model_e06000.pth   # model_best_e09900.pth  # class_model_best_e10001.pth
    ...
    parser.add_argument('-m_t', dest='tissue_model_path', default='/vast/AI_team/sukmin/Results_monai_Lunit_Challenge_Tissue_only/Lunit_Tissue_only_StainNorm_DL_se_ResNext_b8_2_to_1/model_best_e05500.pth') # best 모델로 선정

    # parser.add_argument('-i_c', dest='cell_input_path', default='/vast/AI_team/sukmin/datasets/Lunit_Challenge/Task740_Lunit_Cell_StainNorm_r10_Blob_Cell/imagesTs_circle_150')
    parser.add_argument('-m_c', dest='cell_model_path', default='/vast/AI_team/sukmin/yolov8/r14_only_cell/train8/weights/best.pt')
