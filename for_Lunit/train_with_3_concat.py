...
from core.core_for_lunit_3_concat import train
...
def main(info, config, logging=False):
    ...
    if info["FOLD_for_CrossValidation"] == False:
        train_loader = call_dataloader_Lunit(info, config, train_list, train_tissue_files, train_cell_files, train_transforms, val_transforms, progress=True, mode="train")
        valid_loader = call_dataloader_Lunit(info, config, valid_list, val_tissue_files, val_cell_files, val_transforms, val_transforms, progress=True, mode="valid")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-trainer', default= 'Lunit_Tissue_with_YOLO_StainNorm_rad10_Blob_Cell') # Stroma_200x_only_Epi
