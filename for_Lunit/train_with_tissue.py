...
from core.core_for_lunit import train
...
def main(info, config, logging=False):
    ...
    train_tissue_files = [{"image": join("/vast/AI_team/sukmin/datasets/Lunit_Challenge/Lunit_Tissue_cutting/labelsTr", seg), 
                           "label": join("/vast/AI_team/sukmin/datasets/Lunit_Challenge/Lunit_Tissue_cutting/labelsTr", seg)} for seg in segs]

    val_tissue_files = [{"image": join("/vast/AI_team/sukmin/datasets/Lunit_Challenge/Lunit_Tissue_cutting/labelsTs", seg), 
                         "label": join("/vast/AI_team/sukmin/datasets/Lunit_Challenge/Lunit_Tissue_cutting/labelsTs", seg)} for seg in val_segs]


    # images = sorted(next(os.walk(join(info["Tissue_Path"], "imagesTr")))[2])
    # segs = sorted(next(os.walk(join(info["Tissue_Path"], "labelsTr")))[2])
    # tissue_files = [{"image": join(info["Tissue_Path"], "imagesTr", img), "label": join(info["Tissue_Path"], "imagesTr",seg)} for img, seg in zip(images, segs)]
    ...
    if info["FOLD_for_CrossValidation"] == False:
        train_loader = call_dataloader_Lunit(info, config, train_list, train_tissue_files, train_transforms, val_transforms, progress=True, mode="train")
        valid_loader = call_dataloader_Lunit(info, config, valid_list, val_tissue_files, val_transforms, val_transforms, progress=True, mode="valid")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-trainer', default= 'Lunit_tissue_with_StainNorm_rad10') # Stroma_200x_only_Epi
