    ...
    with torch.no_grad():
        ...
        for img_name in img_names:
            if img_name[-4:] =='.png':
                ...
                # rst = morphology.remove_small_objects(morphology.remove_small_holes(rst>0.5),16)
                ...
                cv2.imwrite(rst, join(args.output_path, 'val_' + img_name.split('_')[-1]))
                t1 = time.time()
                ...
                for x in range(rst2.shape[0]):
                    for y in range(rst2.shape[1]):
                        if rst2[x][y] == 1:
                            vol_1[x][y] = 1
                            vol_2[x][y] = 0
                rst2_1 = measure.label(morphology.remove_small_objects(morphology.remove_small_holes(vol_1>0.5),16))            
                rst2_2 = measure.label(morphology.remove_small_objects(morphology.remove_small_holes(vol_2>0.5),16))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', dest='input_path', default='/vast/AI_team/sukmin/datasets/1.PAIP2023_Validation') # 1.PAIP2023_Validation 
    parser.add_argument('-o', dest='aim_path', default="Val_dpn131_b16_e3.5k") # Val_DeepLabV3+_res101_b32_e14.5k_b
    parser.add_argument('-n', dest='epoch_num', default='model_e02500.pth') # model_e03000.pth # model_best.pth
