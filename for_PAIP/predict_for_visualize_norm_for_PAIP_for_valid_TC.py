...
def main(info, config, args):
    ...
    with torch.no_grad():
        ...
        for img_name in img_names:
            if img_name[-4:] =='.png':
                ...
                # plt.imshow(cv2.bgr2rgb(vol_1_255))
                # plt.show()

                # plt.figure(figsize=(8, 6))
                # plt.imshow(cv2.bgr2rgb(vol_2_255))
                # plt.show()
                ...
                # Watershed 된 이미지를 다시 255 포맷으로 변경

                markers = markers.astype(np.uint8)
                markers2 = markers2.astype(np.uint8)

                rst2_1 = np.zeros(rst2.shape).astype(np.uint8)
                rst2_2 = np.zeros(rst2.shape).astype(np.uint8)
    ...
    # 두 개의 tc_value.csv 파일을 통해 실제 결과 값을 계산해본다.
    import pandas as pd
    import pingouin as pg

    gt_TC_fn = '/vast/AI_team/sukmin/datasets/PAIP_valid_for_me/tc_value.csv'
    pred_TC_fn = join(args.output_path, 'tc_value.csv')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', dest='trainer', default='PAIP_2023_norm')
    parser.add_argument('-i', dest='input_path', default='/vast/AI_team/sukmin/datasets/PAIP_valid_for_me/images') # 1.PAIP2023_Validation 
    parser.add_argument('-o', dest='aim_path', default="Val_CAD_DL_ConvNext_large_b8_bbbaseline_DiceCE_r1024_223_e0.6k") # Val_DeepLabV3+_res101_b32_e14.5k_b
    parser.add_argument('-n', dest='epoch_num', default='model_best_600.pth') # model_e02000.pth # model_best_2500.pth 
