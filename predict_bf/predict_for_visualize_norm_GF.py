        ...
        for img_name in img_names:
            if img_name == 'Thumbs.db':
                ...
                if args.for_sudo_labeling:
                    ...
                    rst2 = np.repeat(rst2[..., np.newaxis], 3, -1)
                    for x in range(rst2.shape[0]):
                        for y in range(rst2.shape[1]):
                            if rst2[x][y][0] == 0:
                                rst2[x][y] = [0, 255, 0]  # back-ground (초록)

                            elif rst2[x][y][0] == 1:      # BGR 
                                rst2[x][y] = [255, 0, 0]  # stroma (파랑)
                            elif rst2[x][y][0] == 2:
                                rst2[x][y] = [0, 255, 255]  # epithelium (노랑)
                            elif rst2[x][y][0] == 3:
                                rst2[x][y] = [66, 129, 245]  # others (주황)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', dest='input_path', default='/vast/AI_team/sukmin/Datasets_for_inference/2022_stomach_200x - Testset') # 2022_stomach_200x - Testset  
    parser.add_argument('-o', dest='aim_path', default="Test_CacoX_b32_DiceCE_GF_0.3_b350") # Test_ResUNet_b256_256_e06k  # 400x
    parser.add_argument('-n', dest='epoch_num', default='model_best_350.pth') # model_e15000.pth # model_best.pth
