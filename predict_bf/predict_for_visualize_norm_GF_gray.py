    ...
    with torch.no_grad():
        for img_name in img_names:
            ...
            if args.for_sudo_labeling:
                ...
                rst2 = rst2.astype(np.uint8)
                
                # rst2 = np.repeat(rst2[..., np.newaxis], 3, -1) # RGB 채널 만드는 과정


                for x in range(rst2.shape[0]):
                    for y in range(rst2.shape[1]):
                        if rst2[x][y] == 1:      # BGR 
                            rst2[x][y] = 50  # stroma (파랑)
                        elif rst2[x][y] == 2:
                            rst2[x][y] = 100  # epithelium (노랑)
                        elif rst2[x][y] == 3:
                            rst2[x][y] = 150  # others (주황)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', dest='trainer', default='Stroma_norm_GF')                                                             # 2022_stomach_400x
    parser.add_argument('-i', dest='input_path', default='/vast/AI_team/sukmin/Datasets_for_inference/TransferTest_NET') # 2022_stomach_200x - Testset  
    parser.add_argument('-o', dest='aim_path', default="TransferTest_NET_CacoX_b32_DiceCE_GF_0.3_b1550") # Test_ResUNet_b256_256_e06k  # 400x
    parser.add_argument('-n', dest='epoch_num', default='model_best_1550.pth') # model_e15000.pth # model_best.pth
    ...
    parser.add_argument('-for_sudo_labeling', required=False, default=True, action="store_true", help='inference for labeling')
    ...
    args.output_path_for_sudo_label = join('/vast/AI_team/sukmin/Results_for_sudo_label_gray', args.aim_path)
